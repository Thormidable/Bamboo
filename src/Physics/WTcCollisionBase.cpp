...
float lfClosestDist=0.0f;
 ...
 for(liLoop=0;liLoop<lpModel->Planes();++liLoop)
 {
  ...
  mvSpherePos+=lpClosestPlane*lfClosestDist;
...
else
{
	...
	if(lfEq[1]<lfSize*lfSize)
	{
        ...
        /*
        lpStart=lpRV+lmBeamMatrix.Position();
        lpStart-=lmSphereMatrix.Position();
        lfSize=lpSphere->CollisionSize()+(lfEq[1]-lfSize)*0.5f;
        lpStart*=(lfSize/lfEq[1]);
        lpTemp->mvCentre+=lpStart;
        lpTemp->mfDistance=lfEq[0];
        */
        cCollisionList::mpStaticList->AddCollision(lpTemp);
	}
...
cBeamCollision *cCollisionBase::SetTypeBeam(cBeamMesh *lpBeam){return SetType(lpBeam);};
