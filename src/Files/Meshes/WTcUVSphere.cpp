...
{
    Generate(liSubdivisions,lfUVSphereSize);
	AddData(lbNormals,UVCoords);
    BufferMesh();
}

cUVSphere::cUVSphere(cUVSphere *lpSphere) : cMesh(lpSphere)
{
...
void cUVSphere::Generate(uint8 liSubdivisions,float lfUVSphereSize)
{
...
void cUVSphere::Radius(float lfSize)
{
  lfSize=lfSize/mpVertex[1];
  for(uint32 liCount=0;liCount<miVertex*3;liCount+=3)
  {
      mpVertex[liCount]*=lfSize;
      mpVertex[liCount+1]*=lfSize;
      mpVertex[liCount+2]*=lfSize;
  }
