...
cFont::cFont(cTextureArray *lpTexture) : cTexture(lpTexture)
{
    /*
...
cFontCharacter::cFontCharacter(cFont *lpFont)
{
 ...
 mpData[IMF_FONT_CHARACTERS].X=((float)(miSize)/(float)lpFont->miWidth);
 mpData[IMF_FONT_CHARACTERS].Y=((float)(miSize)/(float)lpFont->miHeight);
